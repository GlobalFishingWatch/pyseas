...
def format_center(obj):
    ...
    return "{} {}".format(
        format_lon(args["central_longitude"]), format_lat(args["central_latitude"])
    )
...
def build_doc(obj):
    ...
    for k, v in obj.items():
        ...
        if "notes" in v:
            name += "<br/>{}".format(v["notes"])
        v["thumbnail"] = format_thumbnail(k, v)
        v["proj"] = v.get("proj-string", "")
        lines.append("{} | {} | {extent} | {thumbnail}".format(k, name, **v))
    return "\n".join(lines)


if __name__ == "__main__":
    with open("pyseas/data/projection_info.json") as f:
        obj = json.load(f)
    md = build_doc(obj)
    with open("pyseas/doc/projection_info.md", "w") as f:
        f.write(md)
