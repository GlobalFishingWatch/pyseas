...
def colorFromBivariateData(Z1, Z2, z1min_cap, z1max_cap):
    #
    # Get log of each grid cell in Z1 (intensity raster)
    # (if zero, then replace it with the minimum so that log doesn't raise an error)
    Z1 = np.log10(Z1,
                  out=np.ones_like(Z1) * np.log10(np.min(Z1[np.nonzero(Z1)])),
                  where=(Z1 != 0))
    # z1min_cap = Z1.min() + 3.
    # z1max_cap = Z1.max() - 2.
    ...
    Z1_plot = np.array((Z1 - z1min_cap) / (z1max_cap - z1min_cap))
...
def add_raster_bv(raster1, raster2, z1min_cap, z1max_cap,
                  ax=None, extent=(-180, 180, -90, 90), origin='upper',
                  **kwargs):
    """Add a raster to an existing map
    ...
    raster = colorFromBivariateData(raster1, raster2, z1min_cap, z1max_cap)
    #
